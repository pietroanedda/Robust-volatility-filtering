...
filter_beta_t_GARCH <- function(y, theta){
  ...
  u[n] <- score_beta_t_GARCH(y[n], lambda[n], nu)
...
conditional_loglik_beta_t_GARCH <- function(param, data){
...
estimator_beta_t_GARCH <- function(data, param){
  ...
  optimizer <- nlminb(start = param, objective = conditional_loglik_beta_t_GARCH, 
